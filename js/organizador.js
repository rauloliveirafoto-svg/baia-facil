...
(function() {
  ...
  function buildTip(data) {
    ...
    var rows = '';
    if (data.holderName)                        rows += tipRow('Titular',    esc(data.holderName));
    if (data.contactPhone)                      rows += tipRow('Contato',    esc(data.contactPhone));
    if (data.requestedStalls > 0)               rows += tipRow('Qtd. baias', data.requestedStalls);
    if (data.reservedAt)                        rows += tipRow('Reservado',  fmtDate(data.reservedAt));
  ...
  function positionTip(el) {
    ...
    tip.style.top = (r.top - th < 8 ? r.bottom + 8 : r.top - th) + 'px';
  }

  window.BAIA_TOOLTIP = {
    showTip: function(el, data) {
...
/* ── Listener global — declarado fora do DOMContentLoaded ─── */
var _unsub = null;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // ── Construir mapa principal ──────────────────────────────
  window.BAIA_MAP.buildStallMap({ mapElement:elMap, template:elTpl, onStallClick:clickBaia });
  elMap.querySelectorAll('.stall').forEach(function(b) {
    btnMap.set(Number(b.dataset.stallNumber), b);
  });
  iniciarListener();

  // ── Tooltip — apenas mouseover/mouseout (sem mousemove) ───
  elMap.addEventListener('mouseover', function(e) {
    ...
    if (s) window.BAIA_TOOLTIP.showTip(btn, s);
  });
  elMap.addEventListener('mouseout', function(e) {
    if (e.target.closest('.stall')) window.BAIA_TOOLTIP.hideTip();
  });
  ...
  $('btnMoveReservation').addEventListener('click', function() {
    reqSel(function() {
      ...
      editarCache(function(next) {
        ...
        src.status = 'available'; src.holderName = ''; src.contactPhone = '';
        src.requestedStalls = 0; src.reservedAt = '';
        ok = true;
      });
  ...
  function buscarNoMapa() {
    ...
    btn.classList.remove('stall--highlight');
  ...
  $('exportPdf').addEventListener('click', function() { msg('Exportação PDF não disponível nesta versão.', true); });
  ...
  function atualizarBaia(numero, valores) {
    editarCache(function(next) {
      ...
      if (!s) return;
      var st = valores.status !== undefined ? valores.status : s.status;
      s.status          = st;
      s.holderName      = valores.holderName      !== undefined ? valores.holderName      : (st !== 'reserved' ? '' : s.holderName);
      s.contactPhone    = valores.contactPhone    !== undefined ? valores.contactPhone    : (st !== 'reserved' ? '' : s.contactPhone);
      s.requestedStalls = valores.requestedStalls !== undefined ? valores.requestedStalls : (st !== 'reserved' ?  0 : s.requestedStalls);
      s.reservedAt      = st === 'reserved' ? (s.reservedAt || new Date().toISOString()) : '';
    });
    renderTudo();
  }
  ...
  function renderDetalhe() {
    if (!selBaia || !cache) { elNoSel.hidden = false; elDetail.hidden = true; return; }
    var s = cache.stalls.find(function(x) { return x.number === selBaia; });
    if (!s) { elNoSel.hidden = false; elDetail.hidden = true; return; }
  ...
  function renderPagina() {
    ...
    linhas.slice(0, pagina * PAGE).forEach(function(s) {
      var dot = statusColors[s.status] || '#7a9e7e';
    ...
    elTable.querySelectorAll('[data-n]').forEach(function(b) {
      b.addEventListener('click', function() {
        ...
        setTimeout(function() {
          ...
          sb.classList.remove('stall--highlight');
          void sb.offsetWidth;
    ...
    var vis   = Math.min(pagina * PAGE, total);
  ...
  function abrirManual() {
    ...
    elManFeed.textContent = ''; elManFeed.className = 'org-feedback';
    atualizarPrevManual();
    elManMap.innerHTML = '';
    manBtnMap = new Map();
    ...
    renderMapaManual();
    elManModal.hidden = false;
    document.body.style.overflow = 'hidden';
  }

  function fecharManual() {
    elManModal.hidden = true;
  ...
  function atualizarPrevManual() {
    elManPrev.textContent = manSel.length === 0
      ? 'Nenhuma baia selecionada'
      : 'Selecionadas: ' + manSel.slice().sort(function(a,b){return a-b;}).map(fmt).join(', ');
  }
  ...
  function confirmarManual() {
    ...
    msg('Reserva criada: baias ' + manSel.slice().sort(function(a,b){return a-b;}).map(fmt).join(', ') + ' → ' + nome + '.');
  }

  // ── Exportar CSV ──────────────────────────────────────────
  function exportCSV() {
    if (!cache) { msg('Sem dados para exportar.', true); return; }
    var rows = [['Baia','Status','Titular','Telefone','Qtd','Reservado em']];
    cache.stalls.forEach(function(s) {
      rows.push([
        fmt(s.number), statusLabel(s.status), s.holderName || '', s.contactPhone || '',
        s.requestedStalls || '',
        s.reservedAt ? new Date(s.reservedAt).toLocaleString('pt-BR') : '',
      ]);
    });
